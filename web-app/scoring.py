...
def activity_trajec_scoring(df, at_score):
    ...
    slopes = (
        ...
        .apply(slope_score, include_groups=False)
        .reset_index()
    )
...
async def fetch_global_denominators_batch():
    ...
    print("\nFetching global trial landscape in batches of 250...")
...
async def scoring_system(master_df):
    # Await the async fetch 
    global_df = await fetch_global_denominators_batch()
    ...
    # Run the standard math functions
    ...
    browser_download.download_csv(score, "weight_scoring.csv")
    print("Triggered browser download for: weight_scoring.csv")
